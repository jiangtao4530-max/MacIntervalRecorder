...
也可选择“人体走动检测”模式。该模式使用 Apple Vision 在本机识别人形；有人进入画面或人体位置变化超过灵敏度阈值时开始录像，无活动达到设定秒数后停止并保存片段。图像分析不会上传到网络。

默认的“智能混合”模式以设定间隔作为一个周期。周期内检测到人体活动会立即录像，并跳过该周期的定时补录；如果整个周期都没有人体活动，则在周期结束时自动录制一个设定时长的保底片段。若保底录像期间出现人体活动，录像会继续到活动停止。

导出时会自动使用均衡压缩：视频限制在最高 1280 × 720，并优先使用 HEVC 编码；不支持时自动回退到兼容的 720p 编码。压缩和合并均在本机完成。
